...
（1）启动防火墙本节介绍enable和set_uid_rule。其中，enable将调用FirewaCtroner的由上述代码可知，当启动防火墙时，filter表enableFireWa函数，代码如下所示。

中三个用于防火墙控制的Chain的目标都改为[--＞FirewaControer.cpp：​：

DROP或REJECT，这样系统就无法收发网络enableFirewa]数据包。图2-19所示为笔者在Ubuntu机器上设置的规则。
...
[--＞FirewaControer.cpp：​：setUidRule]该函数很简单，就是调用正确的iptables命令，它们分别如下。
...
通过adbshe登录手机时，其实使用的都是伪终端。图2-20所示为笔者用adbshe登录Netd中的logwrap将使用伪终端在父子进程GalaxyNote2后用y命令打印的输出。

中传递log信息。代码如下所示。

[--＞logwrap.c]图2-20y命令对程序员来说，Linux提供了针对伪终端的编程接口。伪终端在概念上和管道没有太大区别，但是在实际操作时有一些特殊API需要调用。
