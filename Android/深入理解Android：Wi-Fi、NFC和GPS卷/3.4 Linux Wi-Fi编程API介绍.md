...
但编程只不过是规范的某种实现，掌握规范才是理解无线网络技术的核心。这也是本书内容组织和编排的指导原则，希望读者能认真体会。

Linux平台上目前常用的专门针对无线网络设[40]备编程的API有两套。

❑最早的一套API由HP公司员工JeanTourrilhes于1997年开发，全称为LinuxWirelessExtensions。一般缩写为wex或wext。这套API使得用户空间的程序能通过ioctl函数来控制无线网卡驱动。
...
1.常用数据结构首先，所有用户空间发起的请求都统一包括在如iwreq结构所示，具体的参数信息存储在另structiwreq中，其原型如下。

外一个联合体iwreq_data中，其原型如下。

[--＞wireless.h：​：structiwreq][--＞wireless.h：​：union：iwreq_data]
...
[--＞wireless.h：​：structiw_scan_req]下面来看最后一个常见的数据结构iw_freq，其原型如下。
...
提示wext中的数据结构和定义还有许多，建议读者结合实际需要去学习wireless.h。

wextAPI虽然简单，但相信读者已经体会到其背后所依赖的和802.11规范密切相关的理论知识了。

下面通过一个实际的例子来看看用户空间如何通过wextAPI来触发无线网卡扫描工作的。
